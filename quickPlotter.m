...
% Last Modified by GUIDE v2.5 19-Apr-2018 22:08:23
...
global hFig;
global hAx;
FM.addFigure(hObject,'quickPlotter');
...
guiData.runCustomFunctions=@runCustomFunctions;
...
set(guiData.FunctionsLstBx,'String',{'Command line'  'Load functions'});
set(guiData.Function2LstBx,'String',{'Command line'  'Load functions'});
set(guiData.CommandLstBx,'String',{'Command line' 'Load commands'});
set(guiData.ShowRegressionRB,'Value',false);
...
%%%% this block is specifically for viewing spm data files
st=rowNames{1};
[~,fname,ext]=fileparts(st);
st=colNames{1};
ind=strfind(st,')');
if(~isempty(ind))
    colNames{1}=[fname ' ' st(1:ind(1))];
end
%%%%
...
guiData.qpData.ShowRegression=get(guiData.ShowRegressionRB,'value');
xLabels=get(guiData.xLabelsTF,'string');
yLabels=get(guiData.yLabelsTF,'string');
pT=get(guiData.PlotTitleTF,'string');
if(~isempty(xLabels))
    guiData.qpData.xLabels=strsplit(xLabels,';');
else
    guiData.qpData.xLabels={};
end
if(~isempty(yLabels))
    guiData.qpData.yLabels=strsplit(yLabels,';');
else
    guiData.qpData.yLabels={};
end

if(~isempty(pT))
    guiData.qpData.PlotTitle=strsplit(pT,';');
else
    guiData.qpData.PlotTitle={};
end
...
guiData=guidata(hObject);
selection=quickPlotterGuiDataHandler.getRowSelection(guiData.qpData);
set(guiData.RowNameLstBx, 'String', names(selection));
...
function updateX1X2YList(hObject)
guiData=guidata(hObject);
selection=quickPlotterGuiDataHandler.getRowSelection(guiData.qpData);
X1=quickPlotterGuiDataHandler.getX1_qpData(guiData.qpData);
X1=arrayfun(@(x) num2str(x), X1(selection), 'UniformOutput', false);

set(guiData.X1LstBx,'String', X1);


qguiData=guidata(hObject);
X2=quickPlotterGuiDataHandler.getX2_qpData(guiData.qpData);
X2=arrayfun(@(x) num2str(x), X2(selection), 'UniformOutput', false);
...
if(~isempty(Y))
    YL=arrayfun(@(x) num2str(x), Y(selection), 'UniformOutput', false);
    guiData=guidata(hObject);
...
%this function update the column index(es) for X1
...
%this function update the column index(es) for X2
if(exist('hObject','var'))
    guiData=guidata(hObject);
...
%this function update the column index(es) for Y
if(exist('hObject','var'))
    guiData=guidata(hObject);
...
for i=1:length(subs)
    sub=subs{i};
    if(get(guiData.ShowCrossHairRB,'Value'))
...
strYgroups=get(guiData.YScaleGroupTF, 'String');
if(isempty(strYgroups))
    groups={1:length(subs)};
else
    groups=CommonMethods.str2numGroups_decimal(strYgroups, ';');
end
...
guiDataF=guidata(guiData.hPlot);
subs=guiDataF.subHandlers;
strXgroups=get(guiData.XScaleGroupTF, 'String');

if(isempty(strXgroups))
    groups={1:length(subs)};
else
    groups=CommonMethods.str2numGroups_decimal(strXgroups, ';');
end
...
%    Axes=cellfun(@(x) num2str(x.hAxs(1)), subs, 'UniformOutput', false);
    Axes=cellfun(@(x) x.hAxs(1), subs, 'UniformOutput', false);%taihao 20160928
...
%    set(guiData.CurrentFigerT, 'String', ['FIGURE: ' num2str(guiData.hPlot)]);
    set(guiData.CurrentFigerT, 'String', ['FIGURE: ' num2str(guiData.hPlot.Number)]);
end
...
function [data, colNames, rowNames]=runCustomFunctions(hObject, data, colNames, rowNames, appendData)
guiData=guidata(hObject);
inds=get(guiData.FunctionsLstBx,'Value');
...
for c=1:length(inds)
    ...
    if(exist(func, 'file'))
        try
            [data, colNames, rowNames]=eval([func '(data, colNames, rowNames);']);
        catch err
            cprintf('red', '%s\n', err.message');
            continue;
        end
...
%            set(guiData.FunctionsLstBx, 'String', horzcat(choices, {func}));
    ...
    updateGui(hObject);
elseif(strcmpi(choice, 'Load functions'))
    scriptdir=fullfile(CommonEnvMethods.getScriptPath(), 'QPfunctions');
    if(~exist(scriptdir, 'file'))
        mkdir(scriptdir);
    end
    fname=uigetfile(fullfile(scriptdir,'*.m'),'select file');
    [path,name, ext]=fileparts(fname);
    choices{end+1}=name;
    set(guiData.FunctionsLstBx,'String',choices);
    guidata(hObject, guiData);
    updateGui(hObject);
end
...
function YScaleGroupTF_Callback(hObject, eventdata, handles)
% hObject    handle to YScaleGroupTF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of YScaleGroupTF as text
%        str2double(get(hObject,'String')) returns contents of YScaleGroupTF as a double


% --- Executes during object creation, after setting all properties.
function YScaleGroupTF_CreateFcn(hObject, eventdata, handles)
% hObject    handle to YScaleGroupTF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function subs=getSelectedSubplots(hObject)
guiData=guidata(hObject);
inds=get(guiData.SubplotLstBx,'Value');
guiDataF=guidata(guiData.hPlot);
subs=guiDataF.subHandlers;
subs=subs(inds);

% --- Executes on button press in SetXScaleBT.
function SetXScaleBT_Callback(hObject, eventdata, handles)
% hObject    handle to SetXScaleBT (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
guiData=guidata(hObject);
subs=getSelectedSubplots(hObject);
lim=get(guiData.AxisScaleTF,'String');

if(~strcmpi(lim,'auto'))
    lim=CommonMethods.str2nums_decimal(lim);
end
...
refresh(hF);
% --- Executes on button press in SetYScaleBT.
function SetYScaleBT_Callback(hObject, eventdata, handles)
% hObject    handle to SetYScaleBT (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
guiData=guidata(hObject);
subs=getSelectedSubplots(hObject);
...
function AxisScaleTF_Callback(hObject, eventdata, handles)
% hObject    handle to AxisScaleTF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of AxisScaleTF as text
%        str2double(get(hObject,'String')) returns contents of AxisScaleTF as a double
updateGui(hObject);

% --- Executes during object creation, after setting all properties.
function AxisScaleTF_CreateFcn(hObject, eventdata, handles)
% hObject    handle to AxisScaleTF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in ShowRegressionRB.
function ShowRegressionRB_Callback(hObject, eventdata, handles)
% hObject    handle to ShowRegressionRB (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of ShowRegressionRB
updateGui(hObject);



function xLabelsTF_Callback(hObject, eventdata, handles)
% hObject    handle to xLabelsTF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of xLabelsTF as text
%        str2double(get(hObject,'String')) returns contents of xLabelsTF as a double
updateGui(hObject);

% --- Executes during object creation, after setting all properties.
function xLabelsTF_CreateFcn(hObject, eventdata, handles)
% hObject    handle to xLabelsTF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function yLabelsTF_Callback(hObject, eventdata, handles)
% hObject    handle to yLabelsTF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of yLabelsTF as text
%        str2double(get(hObject,'String')) returns contents of yLabelsTF as a double
updateGui(hObject);

% --- Executes during object creation, after setting all properties.
function yLabelsTF_CreateFcn(hObject, eventdata, handles)
% hObject    handle to yLabelsTF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function PlotTitleTF_Callback(hObject, eventdata, handles)
% hObject    handle to PlotTitleText (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of PlotTitleText as text
%        str2double(get(hObject,'String')) returns contents of PlotTitleText as a double
updateGui(hObject);

% --- Executes during object creation, after setting all properties.
function PlotTitleTF_CreateFcn(hObject, eventdata, handles)
% hObject    handle to PlotTitleText (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in ShowCrossHairRB.
function ShowCrossHairRB_Callback(hObject, eventdata, handles)
% hObject    handle to ShowCrossHairRB (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of ShowCrossHairRB
updateGui(hObject);
